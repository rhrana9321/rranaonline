...
class New_customer_add extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('superadmin/New_customer_addPage', $data);
	}
	
	public function show_cover_gel_qty()
	{
		$package 			=  $this->input->post('package');
		$result = $this->M_cloud->findbyidstock('package_table', array('id' => $package));
	...
	public function zone_store()
	{
		...
		redirect('software/New_customer_add');
	}
	...
	public function store()
	{
		...
		//if(($data['opening_amount'] > 0) 0r ($data['connect_charge'] > 0)){
		...
		$data3con['customer_id'] 	= $lastid;
		$data3con['payment_date'] 	= date("Y-m-d");
		$data3con['amount'] 		= $data['connect_charge'];
		$data3con['cdate_time'] 	= date('d/m/Y h:i:s a', time());
		$data3con['monthly_name'] 	= $month;
		$data3con['yearly_name'] 	= $year;
		$this->db->insert('connecting_charge_report_table', $data3con);
		//}
		// con end
		
		// opening_amount accpont  statement start //
		if($data['opening_amount'] > 0){
			$text 									=$monthName .' Months Opeaning amount of  ';
			$as_data['customer_id'] 				= $lastid;
			$as_data['client'] 						= $lastid;
			$as_data['credit'] 						= $data['opening_amount'];
			$as_data['debit'] 						= 0;
			$as_data['particular'] 					= $text .$data['name'];
		...
		if($data['connect_charge'] > 0){
			$text2 									=$monthName .' Months connecting charge of  ';
			$asc_data['customer_id'] 				= $lastid;
			$asc_data['client'] 				= $lastid;
			$asc_data['credit'] 					= $data['connect_charge'];
			$asc_data['debit'] 						= 0;
			$asc_data['particular'] 				= $text2 .$data['name'];
		...
		//Running Month Amount statement start //
		if($data['opening_amount'] > 0){
		$Running_Month_data['customer_id'] 	= $lastid;
		$Running_Month_data['payment_date'] = date("Y-m-d");
		$Running_Month_data['amount'] 		=  $data['opening_amount'];
		$Running_Month_data['due_bill_amount'] 		=  $data['previus_months_due'];
		$Running_Month_data['type'] 		= 'Opaning_amount';
		$Running_Month_data['month_name'] 		= $monthName;
		$Running_Month_data['details'] 		= 'Running Month Amount';
		$Running_Month_data['cdate_time'] 	= date('d/m/Y h:i:s a', time());
		$this->db->insert('payment_table', $Running_Month_data);
		}
		//Running Month Amount statement end //
		
		//Connection Charge Amount statement start //
		if($data['connect_charge'] > 0){
		$Connection_Charge_data['customer_id'] 	= $lastid;
		$Connection_Charge_data['payment_date'] = date("Y-m-d");
		$Connection_Charge_data['amount'] 		=  $data['connect_charge'];
		$Connection_Charge_data['type'] 		= 'Connect_charge';
		$Connection_Charge_data['due_bill_amount'] 		=  $data['previus_months_due'];
	...
	public function updated()
	{
		...
		redirect('software/Create_Divisional_Admin');
