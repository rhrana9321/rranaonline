...
class Connection_Charge extends CI_Controller {
	...
	public function index()
	{
		...
		$data['all_customer_List'] = $this->M_cloud->findAll('connecting_charge_report_table', 'connecting_report_id  desc');
		$data['success'] = $this->session->userdata('success');
		$this->session->set_userdata(array('success' => ''));
		$this->load->view('superadmin/Connection_ChargePage', $data);
	}
	
	
	public function Connection_ChargePrint()
	{
		$data['title'] = "Customer Manage";
		$this->session->userdata('id');
		$data['superdetails'] = $this->M_cloud->findbyidstock('superadmin', array('id'=> '1'));
		...
		$data['success'] = $this->session->userdata('success');
		$this->session->set_userdata(array('success' => ''));
		$this->load->view('superadmin/Connection_ChargePrintPage', $data);
	}
	...
	public function date_to_date()
	{
		...
		$data['from_date'] 			= $from;
		$data['to_date'] 			= $to;
		$data['all_customer_List'] = $this->M_cloud->minimamaqty('connecting_charge_report_table', array('payment_date >=' => $from_newDate, 'payment_date <=' => $to_newDate), 'connecting_report_id asc');
		$this->load->view('superadmin/show_connecation_charhe_reportPage', $data);
	}
