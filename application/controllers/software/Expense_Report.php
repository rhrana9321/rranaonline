...
class Expense_Report extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('superadmin/Expense_ReportPage', $data);
	}
	...
	public function Expense_ReportPrint()
	{
		$header_name = $this->input->post('id_header_name');
		$data['superdetails'] = $this->M_cloud->findbyidstock('superadmin', array('id'=> '1'));
		$data['infoList'] = $this->M_cloud->findbyidstock('account_head_table', array('id'=> $header_name));
		$date1 	= date("Y-m-d");
		$date1 	= explode('-', $date1); 
		...
		$data['account_head_List'] = $this->M_cloud->findAll('account_head_table', 'id desc');
		$data['expense_details_info'] = $this->M_cloud->minimamaqty('expense_table', array('header_name' => $header_name, 'month_name' => $month), 'id desc');
		$this->load->view('superadmin/Expense_detailsPrintPage', $data);
	}
